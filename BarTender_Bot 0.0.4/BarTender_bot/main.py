...
from Handlers.User_handler import user_log_in, User
...
def bot_restarted_command(message):
  ...
  bot.send_message(message.chat.id, 'md', reply_markup=mar.set_markup)
...
def message_text(message):
  ...
  id = message.chat.id
  if text == 'Бар':
    markup = Markup('bs, md, a, b', 'Бармен, Кладовщик, Менеджер, Директор, Админ, Бармен', id)
    bot.send_message(id, 'Бар', reply_markup=markup.set_markup)
